...
def _ref() -> tuple[str, str]:
    ...
    return f'{registry}/kubewi/ros-motion:{tag}', registry


def register(sub) -> None:
    p = sub.add_parser('ros-motion', help='Workload ROS 2 control — nœuds moteur (FROM ros-core)')
    s = p.add_subparsers(dest='ros_motion_cmd', metavar='CMD', required=True)
