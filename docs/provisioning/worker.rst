...
que pendant l'enrollment d'un nouveau worker via :
...
temporaire (``192.168.0.x``) qui permet le provisioning Ansible via
...
| Rôle : `src/eng_rpios/roles/rpios/tasks/cgroups.yml <https://github.com/iobewi/kubewi/blob/main/src/eng_rpios/roles/rpios/tasks/cgroups.yml>`_

Sur Raspberry Pi, les cgroups mémoire ne sont pas activés par défaut dans
le bootloader. k0s les requiert. Le rôle rpios les active automatiquement
...
.. code-block:: bash
   ...
   kubewi vpn up
   kubewi ssh init    # génère la clé SSH et la pousse sur tous les nœuds
   kubewi k0s kubeconfig

``kubewi ssh init`` est nécessaire uniquement lors du premier setup ou après
un rebuild du container SDK (la clé ``~/.ssh/kubewi_ansible`` n'est pas
persistée dans l'image). Si la clé est déjà en place, ``kubewi ssh config``
suffit pour reconfigurer ``~/.ssh/config``.

``kubewi k0s kubeconfig`` récupère le kubeconfig k0s depuis le controller et
...
La commande suivante gère l'intégralité du processus : activation du DHCP
de provisioning, détection des baux dnsmasq, ajout dans ``hosts.yml``,
enrollment Ansible en deux phases, désactivation du DHCP.

.. code-block:: bash

   kubewi enroll worker

Le script ``src/adp_ansible/scripts/enroll.py`` :

1. Scale le Deployment ``dnsmasq-provisioning`` à 1
...
6. Scale le Deployment à 0 en cas de succès
...
.. list-table::
   ...
   * - Option
     - Effet
   * - ``--single``
     - Détecte un seul nœud, enrollment immédiat
   * - ``--yes``
...
.. code-block:: bash
   ...
   kubewi enroll worker --single --yes
...
| Playbook : `src/eng_k0s/playbooks/workers-init.yml <https://github.com/iobewi/kubewi/blob/main/src/eng_k0s/playbooks/workers-init.yml>`_
...
2. **Rôle** ``debian`` — OS, modules noyau, sysctl, swap, SSH durci, chrony, containerd
3. **Rôle** ``rpios`` — cgroups RPi, cloud-init désactivé *(si Raspberry Pi OS)*
4. **Rôle** ``network`` — désactivation dhcpcd/NetworkManager/networking,
   création bridge ``br0``, VLANs (220/420/620)
5. **Rôle** ``internal`` — route par défaut ``192.168.22.1`` sur VLAN 220,
   ``resolv.conf`` pointant vers ``192.168.22.1`` (DNS cluster)
6. Attente de disponibilité sur l'IP VLAN 220 (port 22, timeout 60 s)
...
| Playbook : `src/eng_k0s/playbooks/worker.yml <https://github.com/iobewi/kubewi/blob/main/src/eng_k0s/playbooks/worker.yml>`_
| Rôle : `src/eng_k0s/roles/k0s_worker <https://github.com/iobewi/kubewi/blob/main/src/eng_k0s/roles/k0s_worker>`_
...
| Rôle : `src/eng_k0s/roles/k0s_install <https://github.com/iobewi/kubewi/blob/main/src/eng_k0s/roles/k0s_install>`_
...
``src/adp_ansible/inventory/group_vars/kubernetes.yml``. Si non définie,
la dernière release stable est installée automatiquement.
...
| Rôle : `src/eng_k0s/roles/k0s_worker/tasks/registry_config.yml <https://github.com/iobewi/kubewi/blob/main/src/eng_k0s/roles/k0s_worker/tasks/registry_config.yml>`_
...
| Rôle : `src/eng_k0s/roles/k0s_worker/tasks/token.yml <https://github.com/iobewi/kubewi/blob/main/src/eng_k0s/roles/k0s_worker/tasks/token.yml>`_
...
| Rôle : `src/eng_k0s/roles/k0s_worker/tasks/service.yml <https://github.com/iobewi/kubewi/blob/main/src/eng_k0s/roles/k0s_worker/tasks/service.yml>`_
...
.. code-block:: bash
   ...
   kubewi vpn up
   kubewi k0s add worker --name <nom> --check
