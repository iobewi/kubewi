...
def test_deploy_runs_gateway_playbook(mock_subprocess):
...
def test_deploy_uses_ansible_playbook(mock_subprocess):
    from plg_gateway.kubewi.commands import run_cmd
    run_cmd(Namespace(gateway_cmd="deploy"))
    assert any("ansible-playbook" in c for c in _cmds(mock_subprocess))


def test_wifi_deploy_runs_wifi_playbook(mock_subprocess):
    from plg_gateway.kubewi.commands import run_cmd
    run_cmd(Namespace(gateway_cmd="wifi-deploy"))
    assert any("wifi.yml" in c for c in _cmds(mock_subprocess))


def test_wifi_deploy_uses_ansible_playbook(mock_subprocess):
    from plg_gateway.kubewi.commands import run_cmd
    run_cmd(Namespace(gateway_cmd="wifi-deploy"))
    assert any("ansible-playbook" in c for c in _cmds(mock_subprocess))


def test_deploy_and_wifi_deploy_are_independent(mock_subprocess):
