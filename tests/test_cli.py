...
- discover() trouve tous les paquets
...
from conftest import PKG_DIRS, SRC_DIR
...
def test_discovery_finds_all_packages() -> None:
    """discover() doit retourner un module pour chaque paquet connu."""
...
def test_name_is_non_empty_string(pkg_dir: Path) -> None:
    mod = _load_commands(pkg_dir)
    name = getattr(mod, "NAME", None)
    names = getattr(mod, "NAMES", None)
    assert name is not None or names is not None, \
        f"{pkg_dir.name}/commands.py : ni NAME ni NAMES défini"
    if name is not None:
        assert isinstance(name, str) and name, \
            f"{pkg_dir.name}: NAME doit être une chaîne non vide"
    if names is not None:
        assert isinstance(names, list) and all(isinstance(n, str) for n in names), \
            f"{pkg_dir.name}: NAMES doit être une liste de chaînes"


def test_register_is_callable(pkg_dir: Path) -> None:
    mod = _load_commands(pkg_dir)
    assert callable(getattr(mod, "register", None)), \
        f"{pkg_dir.name}/commands.py : register() non callable"


def test_run_cmd_is_callable(pkg_dir: Path) -> None:
    mod = _load_commands(pkg_dir)
    assert callable(getattr(mod, "run_cmd", None)), \
        f"{pkg_dir.name}/commands.py : run_cmd() non callable"


# ── Intégration argparse ───────────────────────────────────────────────────────


def test_register_does_not_crash(pkg_dir: Path) -> None:
...
def test_help_exits_zero(pkg_dir: Path) -> None:
    ...
    assert exc.value.code == 0, \
        f"{pkg_dir.name}: --help a retourné le code {exc.value.code}"
